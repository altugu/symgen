...
public:
	...
	Mesh(bool sm, int i) : secondMesh(sm), id(i), isometricDistortion(9.0f), radius(-1.0f) { color = new float[3]; color[0] = color[1] = color[2] = 0.7f; };\
	void setFileName(string fileName){
		this->fileName = fileName;
	}
	void findNearestMeshtoCom(bool brushVertex, Color color);
	void outputWithBrush();
	void brushSamples();
	void loadObj(char* meshFile);
	void loadOff(char* meshFile, Load_option options, Color specifiedColor = Color(-1,-1,-1));
	void loadPly(char* meshFile);
	void computeEvenlySpacedSamples(int N, bool earlyBreak = false,  Color sampleColor = Color(-1,-1,-1), int startVertex = -1);
	...
	void resultToFile(char* fName, Mesh* mesh2, bool removeOutliers, bool rlComputeMode);
	...
	void adaptiveSampling3(Mesh* mesh2, bool removeOutliers);
...
private:
	...
	void addTriangle(int v1i, int v2i, int v3i, Color color = Color(-1,-1,-1));
