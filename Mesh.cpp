...
void Mesh::findNearestMeshtoCom(bool brushVertex, Color color){

	Vertex* min_vertex = nullptr;
	float min_dist;
	bool flag = true; //min dist
	for(int i=0; i<verts.size(); i++){
		Vertex* vert = verts[i];
		float dist = sqrt(pow((com[0] - vert->coords[0]), 2) + pow((com[1] - vert->coords[1]), 2) + pow((com[2] - vert->coords[2]), 2));
		cout << "dist: " << dist << endl;
		if(flag){
			cout << "a";
			min_vertex = vert;
	...
	if(min_vertex){
		cout << "min_vertex 0: " << min_vertex->coords[0] << " 1: " << min_vertex->coords[1] << " 2: " << min_vertex->coords[2];
		if(brushVertex){
		min_vertex->sample = true;
		min_vertex->processed = true; //added, so not add again later; even if not added ('cos isolated) make processed=true to discard this in upcoming iters as a sample candidate
		
		for(int i = 0; i < min_vertex->triNeighbors.size();i++){
			Triangle* brush_triangle = tris[min_vertex->triNeighbors.at(i)];
			cout << "-----------------" << color.red << " " << color.green << " " << color.blue;
			brush_triangle->setColor(color);
		}
	}
	}else{
		cout << "min_vertex = null" << endl;
	}
}
void Mesh::outputWithBrush(){
	...
	if(modifiedFile.is_open()){
		...
		while(getline(originalFile, line)){
			...
			if(elements.size() == 4 || elements.size() == 7){
				originalFile.close();
				for(int i=0; i<tris.size(); i++){
					Triangle* triangle = tris.at(i);
					modifiedFile << 3;
					modifiedFile << " " << triangle->v1i << " " << triangle->v2i << " " << triangle->v3i;
					if(triangle->sample){
						cout << "sample" << endl;
						if(triangle->color.red > 0) cout<<triangle->idx<<endl;
						modifiedFile << " " << triangle->color.red << " " << triangle->color.green << " " << triangle->color.blue;
					}
					modifiedFile << "\n";
				}
				modifiedFile.close();
...
void Mesh::loadObj(char* meshFile)
{
	...
	while (fscanf(fPtr, "%c %f %f %f\n", &type, &a, &b, &c) != EOF){
		cout << "while " << counter << "type: " << type << endl;
...
void Mesh::loadOff(char* meshFile, Load_option options, Color specifiedColor)
{
#ifdef VERBOSE
	cout << "Mesh initializing (to model in " << meshFile << " file)...\t";
#else
	...
	while(getline(file, line)){
		if(ctr == 0){
			...
			if(elements.size() == 3){ //take coordinates (vertex)
				float* coords = new float[3];
				coords[0] = stof(elements[0]);
				coords[1] = stof(elements[1]);
				coords[2] = stof(elements[2]);
				addVertexND(coords);
			}
			else if(options == Load_option::withoutColor){
				addTriangle(
					(int) stoi(elements[1]),
					(int) stoi(elements[2]),
					(int) stoi(elements[3])
			...
			else if(options == Load_option::withSpecifiedColor){
					addTriangle(
					(int) stoi(elements[1]),
					(int) stoi(elements[2]),
					(int) stoi(elements[3]),
			...
			else if(options == Load_option::withExistingColor){ //take faces - # v1 v2 v3 r g b
				if(elements.size() == 7)
					addTriangle(
						(int) stoi(elements[1]),
						(int) stoi(elements[2]),
...
void Mesh::addTriangle(int v1i, int v2i, int v3i, Color color)
{
	//add tri v1i-v2i-v3i to the triInd'th element of tris

	int triInd = (int) tris.size();
	Triangle* triangle = new Triangle(triInd, v1i, v2i, v3i);
	if((color.red >= 0)){
...
inline float maxOf(float a, float b) { return (a > b ? a : b); }
...
void Mesh::computeEvenlySpacedSamples(int N, bool earlyBreak, Color sampleColor, int startVertex)
{
	...
	while ((int) samples.size() < N)
	{
		...
		Vertex* sample_vertex = verts[nextSample];
		sample_vertex->sample = true;
		sample_vertex->processed = true; //added, so not add again later; even if not added ('cos isolated) make processed=true to discard this in upcoming iters as a sample candidate
		
		for(int i = 0; i < sample_vertex->triNeighbors.size();i++){
			Triangle* brush_triangle = tris[sample_vertex->triNeighbors.at(i)];
			//cout << "-----------------" << sampleColor.red << " " << sampleColor.green << " " << sampleColor.blue;
			brush_triangle->setColor(sampleColor);
		}
...
void Mesh::resultToFile(char* fName, Mesh* mesh2, bool removeOutliers, bool rlComputeMode)
{
...
#else
	cout << "Diso (based on robustList) = " << isometricDistortion << "; Diso (comparable w/ other methods) = " << isoDistoNoRL << " && Dgrd = " << groundDistoFull << "\n";
#endif
...
void Mesh::resultFromFile(char* fName, Mesh* mesh2)
{
	...
	//sprintf(fName, "output\\robustList\\%d-%d.dat", id, mesh2->id);
	sprintf(fName, "input\\robustMaps\\%d-%d.dat", id, mesh2->id);
...
//	if (finalCall)
	...
	char fName[250]; sprintf(fName, "output\\%d-%das.dat", id, mesh2->id);
...
void Mesh::adaptiveSampling2(Mesh* mesh2)
{
	...
	for (int i = 0; i < (int) robustList.size(); i += 2)
	{
		...
		mesh2->computeEvenlySpacedSamples(N, false, Color(56,56,56), robustList[i+1]);
	}
...
void Mesh::adaptiveSampling3(Mesh* mesh2, bool removeOutliers)
{
...
#endif
	char fName[250]; sprintf(fName, "output\\%d-%d ga+as.dat", id, mesh2->id);
	resultToFile(fName, mesh2, removeOutliers, false); //as: adaptive sampling
}
...
/*			for (int i = 0; i < m; i++)
	...
	char fOutName[250];
	sprintf(fOutName, "output\\%d-%ddense.dat", id, mesh2->id);
...
void Mesh::bruteForceMap(Mesh* mesh2, int r)
{
	...
	sprintf(fName, "C:\\PhD\\Dropbox_koc\\my codes\\tools\\%d!.txt", r); //matlab code that generated this txt is in that folder
	if (! (fPtr = fopen(fName, "r")))
	{
...
//cout << optimalMap[0] << " - " << optimalMap[1] << endl;//crushes if inited to an empty vector (vector< int > optimalMap;)
	...
	sprintf(fName, "output\\%d-%d.dat", id, mesh2->id);
